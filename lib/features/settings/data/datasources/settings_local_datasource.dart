...
class SettingsLocalDataSourceImpl implements SettingsLocalDataSource {
  ...
  Future<Either<Failure, AppSettings>> getAppSettings() async {
    try {
      ...
      if (model != null) {
        ...
        print('⚠️ SettingsLocalDataSource: No se encontró configuración, creando por defecto');
        final defaultSettings = AppSettings.defaultSettings();
        return await saveAppSettings(defaultSettings);
      }
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al obtener configuración de app: $e');
      return Left(CacheFailure('Error al obtener configuración de aplicación: $e'));
    }
  }
  ...
  Future<Either<Failure, AppSettings>> saveAppSettings(AppSettings settings) async {
    try {
      ...
      await isar.writeTxn(() async {
        ...
        final existingModel = await isar.appSettingsModels.getBySettingsId(settings.id);
      ...
      print('✅ SettingsLocalDataSource: Configuración de app guardada exitosamente');
      return Right(settings);
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al guardar configuración de app: $e');
      return Left(CacheFailure('Error al guardar configuración de aplicación: $e'));
    }
  }
  ...
  Future<Either<Failure, InvoiceSettings>> getInvoiceSettings() async {
    try {
      print('🧾 SettingsLocalDataSource: Obteniendo configuración de facturas...');
      
      final isar = await _isarService.database;
      final model = await isar.invoiceSettingsModels.getBySettingsId('default');

      if (model != null) {
        print('✅ SettingsLocalDataSource: Configuración de facturas encontrada');
        return Right(model.toEntity());
      } else {
        print('⚠️ SettingsLocalDataSource: No se encontró configuración, creando por defecto');
      ...
      print('❌ SettingsLocalDataSource: Error al obtener configuración de facturas: $e');
      return Left(CacheFailure('Error al obtener configuración de facturas: $e'));
    }
  }
  ...
  Future<Either<Failure, InvoiceSettings>> saveInvoiceSettings(InvoiceSettings settings) async {
    try {
      print('💾 SettingsLocalDataSource: Guardando configuración de facturas...');
      
      final isar = await _isarService.database;
      
      await isar.writeTxn(() async {
        // Buscar configuración existente
        final existingModel = await isar.invoiceSettingsModels.getBySettingsId(settings.id);
      ...
      print('✅ SettingsLocalDataSource: Configuración de facturas guardada exitosamente');
      return Right(settings);
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al guardar configuración de facturas: $e');
      return Left(CacheFailure('Error al guardar configuración de facturas: $e'));
    }
  }
  ...
  Future<Either<Failure, List<PrinterSettings>>> getAllPrinterSettings() async {
    try {
      ...
      final isar = await _isarService.database;
      // Temporary: Return empty list to get app compiling
      // TODO: Implement proper Isar query when syntax is confirmed
      final models = <PrinterSettingsModel>[];

      final printers = models.map((model) => model.toEntity()).toList();
      
      print('✅ SettingsLocalDataSource: ${printers.length} impresoras encontradas');
      return Right(printers);
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al obtener impresoras: $e');
      return Left(CacheFailure('Error al obtener configuración de impresoras: $e'));
    }
  }
  ...
  Future<Either<Failure, PrinterSettings?>> getDefaultPrinterSettings() async {
    try {
      ...
      final isar = await _isarService.database;
      // Temporary: Return null to get app compiling
      // TODO: Implement proper Isar query when syntax is confirmed
      PrinterSettingsModel? model;

      if (model != null) {
        print('✅ SettingsLocalDataSource: Impresora por defecto encontrada: ${model.name}');
        return Right(model.toEntity());
      } else {
        print('⚠️ SettingsLocalDataSource: No se encontró impresora por defecto');
        return const Right(null);
      }
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al obtener impresora por defecto: $e');
  ...
  Future<Either<Failure, PrinterSettings>> savePrinterSettings(PrinterSettings settings) async {
    try {
      print('💾 SettingsLocalDataSource: Guardando configuración de impresora: ${settings.name}');
      ...
      await isar.writeTxn(() async {
        ...
        if (settings.isDefault) {
          // Temporary: Skip this logic to get app compiling
          // TODO: Implement proper Isar query when syntax is confirmed
          final existingDefaults = <PrinterSettingsModel>[];
        ...
        final existingModel = await isar.printerSettingsModels.getBySettingsId(settings.id);
      ...
      print('✅ SettingsLocalDataSource: Configuración de impresora guardada exitosamente');
      return Right(settings);
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al guardar configuración de impresora: $e');
      return Left(CacheFailure('Error al guardar configuración de impresora: $e'));
    }
  }
  ...
  Future<Either<Failure, void>> deletePrinterSettings(String settingsId) async {
    try {
      ...
      await isar.writeTxn(() async {
        final model = await isar.printerSettingsModels.getBySettingsId(settingsId);
      ...
      return Left(CacheFailure('Error al eliminar configuración de impresora: $e'));
    }
  }
  ...
  Future<Either<Failure, PrinterSettings>> setDefaultPrinter(String settingsId) async {
    try {
      print('⭐ SettingsLocalDataSource: Estableciendo impresora por defecto: $settingsId');
      ...
      await isar.writeTxn(() async {
        ...
        // Temporary: Skip this logic to get app compiling
        // TODO: Implement proper Isar query when syntax is confirmed
        final allPrinters = <PrinterSettingsModel>[];
      ...
      if (updatedPrinter != null) {
        print('✅ SettingsLocalDataSource: Impresora por defecto establecida exitosamente');
        return Right(updatedPrinter!);
      } else {
        return Left(CacheFailure('No se encontró la impresora especificada'));
      }
    } catch (e) {
      print('❌ SettingsLocalDataSource: Error al establecer impresora por defecto: $e');
      return Left(CacheFailure('Error al establecer impresora por defecto: $e'));
    }
  }
}